...
(async () => {
  ...
  for (const vp of viewports) {
    ...
    const heroVisible = await p.locator('text=Next Appointment').isVisible().catch(() => false);
    const historyVisible = await p.locator('text=History').isVisible().catch(() => false);
    const nextUpStat = await p.locator('text=Next Up').isVisible().catch(() => false);
